...
`SpiFlashIdentificationSession` 实现原版“部分 25 系列 SPI Flash”识别入口，与普通烧录操作
保持隔离：

1. 每次识别都强制下载并复验内置 `M25ID.alg`，不复用先前算法；
2. 不发送 `0x008A` 或 `SPRJ` 器件参数；
3. 以 `0x000F` 在命令偏移 `+0x08` 写入 330 或 180 厘伏；
4. 探测通过后以 `0x0012` 请求并从 Pipe `0x83` 读取精确 8 字节；
5. 保留完整回包，其中 `+0x00..+0x01` 是检测引脚类，`+0x02..+0x03` 仍为未知字节，
   `+0x04..+0x07` 用于器件 ID 前缀比较；
6. 只接受 `SP20.dev record[0x22] == 0` 的记录，并按首个零字节截断其 1～4 字节 ID；
7. 封装类通常取 `PkgID20` 类型值低字节，`0x1218/0x1219` 特别归一为 16；
8. 返回零到多个候选，调用方必须根据丝印和封装显式确认，识别本身不会选择 profile。

`0x000F` 的拒绝分支从 Pipe `0x83` 读取 64 字节诊断，原始状态和诊断均保留。命令不会调用
擦除、编程或配置写入路径。CLI 入口为 `flypro device identify`，默认 3.3 V，并支持 JSON
输出完整原始结果与候选。
